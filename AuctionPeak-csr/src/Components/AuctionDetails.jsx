...
const AuctionDetails = () => {
  ...
  const auctionDetails = auction?.message?.AuctionDetail
  const auctionItemDetails = auction?.message?.items
  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      {auctionDetails ? (
        <header>
          <h2 className="text-xl font-bold text-gray-900 sm:text-3xl">{auctionDetails?.name}</h2>
